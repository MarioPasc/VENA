...
class pGAN(BaseModel):
    ...
    def forward(self):
        # Patch (VENA): Variable() is a no-op in torch 2.x; tensors carry autograd directly.
    ...
    def test(self):
        # Patch (VENA): Variable(..., volatile=True) was removed in torch 0.4.
        # Use torch.no_grad() at the call site for inference.
        with torch.no_grad():
            self.real_A = self.input_A
            self.fake_B = self.netG(self.real_A)
            self.real_B = self.input_B
    ...
    def get_current_errors(self):
        # Patch (VENA): .data[0] was removed in torch 0.4+; use .item().
