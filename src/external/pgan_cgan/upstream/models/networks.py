...
# Patch (VENA): non-in-place init.* removed in torch 0.4+; switch to in-place _ variants.
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        # Patch (VENA): Variable() is a no-op in torch 2.x; tensors with requires_grad=False
        # are the default. Mirror device of `input` because self.Tensor may not match.
        target_tensor = None
        if target_is_real:
            ...
            if create_label:
                ...
                self.real_label_var.requires_grad_(False)
            ...
            if create_label:
                ...
                self.fake_label_var.requires_grad_(False)
