...
class EMA(Optimizer):
    def __init__(self, opt, ema_decay):
        ...
        # VENA patch P1 (torch >= 2.0 compat): EMA does not call super().__init__,
        # so the Optimizer base-class hook attrs introduced in torch 2.x are
        # missing. state_dict() / load_state_dict() / step() rely on them.
