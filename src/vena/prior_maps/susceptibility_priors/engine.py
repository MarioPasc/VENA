"""Orchestrator for the susceptibility-prior routine.

Mirrors :mod:`vena.prior_maps.perfusion_priors.engine`. Source modality is
``SWI_bias`` (the bias-corrected SWAN magnitude); primary collage channel is
``sus``; the contour traces the soft-response threshold from the model.
"""
...
from vena.prior_maps.susceptibility_priors._collage import render_collage
from vena.prior_maps.susceptibility_priors.abc_model import SusceptibilityInput
from vena.prior_maps.susceptibility_priors.models import MODEL_REGISTRY
...
@dataclass(frozen=True)
class SusceptibilityPriorsRoutineConfig:
    dataset_root: Path
    ...
    @classmethod
    def from_yaml(cls, path: Path | str) -> SusceptibilityPriorsRoutineConfig:
        path = Path(path)
...
class SusceptibilityPriorsEngine:
    SOURCE_MODALITY = "SWI_bias"
    PRIMARY_CHANNEL = "sus"
    ...
    def _run_one_algorithm(
        ...
        *,
        ...
        if algo.name not in MODEL_REGISTRY:
            raise KeyError(
                f"Unknown susceptibility model {algo.name!r}; available: {sorted(MODEL_REGISTRY)}"
            )
        ...
        for p in patients:
            ...
            swi = dataset.load_modality(p, self.SOURCE_MODALITY)
            ...
            channel_paths: dict[str, Path] = {
                "sus": pat_dir / "sus.nii.gz",
                "itss": pat_dir / "itss.nii.gz",
            }
            binary_path = pat_dir / "sus_threshold.nii.gz"
            ...
            if figures_only:
                ...
                for name, arr in channels.items():
                    save_nii(arr, swi.affine, swi.header, channel_paths[name])
                    _assert_affine_preserved(channel_paths[name], swi.affine)
                if binary is not None:
                    save_nii(binary, swi.affine, swi.header, binary_path)
                    _assert_affine_preserved(binary_path, swi.affine)
    ...
    def _persist_provenance(
        ...
        run_dir: Path,
        manifest: dict[str, Any],
        cfg: SusceptibilityPriorsRoutineConfig,
    ) -> None:
