"""Orchestrator for the cellularity-prior routine.

Mirrors :mod:`vena.prior_maps.perfusion_priors.engine` with three differences:
the source modality is ADC, the primary collage channel is ``cell``, and the
contour traces the tumour-mask boundary.
"""
...
from vena.prior_maps.cellularity_priors._collage import render_collage
from vena.prior_maps.cellularity_priors.abc_model import CellularityInput
from vena.prior_maps.cellularity_priors.models import MODEL_REGISTRY
...
@dataclass(frozen=True)
class CellularityPriorsRoutineConfig:
    dataset_root: Path
    ...
    @classmethod
    def from_yaml(cls, path: Path | str) -> CellularityPriorsRoutineConfig:
        path = Path(path)
...
class CellularityPriorsEngine:
    SOURCE_MODALITY = "ADC"
    PRIMARY_CHANNEL = "cell"
    ...
    def _run_one_algorithm(
        ...
        *,
        ...
        if algo.name not in MODEL_REGISTRY:
            raise KeyError(
                f"Unknown cellularity model {algo.name!r}; available: {sorted(MODEL_REGISTRY)}"
            )
        ...
        for p in patients:
            ...
            adc = dataset.load_modality(p, self.SOURCE_MODALITY)
            ...
            channel_paths: dict[str, Path] = {
                "adc_rel": pat_dir / "adc_rel.nii.gz",
                "cell": pat_dir / "cell.nii.gz",
            }
            binary_path = pat_dir / "tumour_mask.nii.gz"
            ...
            if figures_only:
                ...
                binary: np.ndarray | None = (
                    load_nii(binary_path).array.astype(np.uint8, copy=False)
                ...
                for name, arr in channels.items():
                    save_nii(arr, adc.affine, adc.header, channel_paths[name])
                    _assert_affine_preserved(channel_paths[name], adc.affine)
                if binary is not None:
                    save_nii(binary, adc.affine, adc.header, binary_path)
                    _assert_affine_preserved(binary_path, adc.affine)
            ...
            algo_record["patients"].append(
                {
                    "patient_id": p.patient_id,
                    "adc_path": str(adc.path),
                    "adc_sha256": _sha256(adc.path),
    ...
    def _persist_provenance(
        ...
        run_dir: Path,
        manifest: dict[str, Any],
        cfg: CellularityPriorsRoutineConfig,
    ) -> None:
