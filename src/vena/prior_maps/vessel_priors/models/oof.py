...
:mod:`vena.prior_maps.vessel_priors.preprocessing.anisotropic_diffusion`).
...
from vena.prior_maps.vessel_priors.abc_model import (
    AbstractVesselModel,
    VesselInput,
    VesselOutput,
)
...
class OOFVesselModel(AbstractVesselModel):
    ...
    def _radii_voxels(self, spacing_mm: tuple[float, float, float]) -> NDArray[np.float64]:
    ...
    def _oof_multiscale(
        ...
        for r in radii:
            ...
            for i, j, ai, aj in pairs:
                ...
                qij = np.real(ifftn(spectrum * kernel, norm="backward")).astype(np.float32)
                comps[(i, j)] = qij
            ...
            if self.black_ridges:
                response = np.maximum(0.0, -(l_a + l_b) * np.float32(0.5)) * area_norm
            else:
                response = np.maximum(0.0, (l_b + l_c) * np.float32(0.5)) * area_norm
    ...
    def predict(self, x: VesselInput) -> VesselOutput:
        ...
        if brain.shape != swi.array.shape:
            raise ValueError(f"brain mask shape {brain.shape} != SWI shape {swi.array.shape}")
