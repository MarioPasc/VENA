"""Per-cohort and multi-cohort latent-H5 datasets for T1C-RFlow.
...
Departures from the pGAN dataset
--------------------------------
- Reads **latent H5** (``cohort.latent_h5``), not image H5. The conditioning
  latents ``z_T1pre`` and ``z_FLAIR`` and the target ``z_T1c`` are stored under
  ``latents/{t1pre,flair,t1c}`` at shape ``(N, C, h, w, d)``.
- Operates at **volume granularity** (one item = one patient), not slice
  granularity. T1C-RFlow is a 3D model.
- No ``image_size`` / ``pad_to_size`` step — the latents come pre-shaped from
  the encode pipeline and the U-Net consumes them as-is.

Carry-overs from pGAN that must remain identical
------------------------------------------------
...
Citation (every derived artefact must carry it — `UPSTREAM.md` block)
--------------------------------------------------------------------
...
class T1CRFlowLatentDataset(Dataset[dict[str, torch.Tensor | str]]):
    """Per-cohort latent-H5 reader for T1C-RFlow training.
    ...
    input_latents:
        ...
        — paper-faithful for T1C-RFlow (T1pre + the FLAIR surrogate for
        BraTS T2-FLAIR; see ``UPSTREAM.md`` "Scope of use").
    target_latent:
    ...
    def __init__(
        ...
        logger.info(
            "T1CRFlowLatentDataset[%s/fold%d]: %d patients, cond=%s → %s",
...
class MultiCohortT1CRFlowLatentDataset(Dataset[dict[str, torch.Tensor | str]]):
    """ConcatDataset over per-cohort ``T1CRFlowLatentDataset`` instances.
    ...
    def __init__(
        ...
        datasets: list[T1CRFlowLatentDataset] = []
        ...
        for entry in cohorts:
            ...
            if "latent_h5" not in entry:
                logger.warning(
                    "MultiCohortT1CRFlowLatentDataset: skipping cohort %s — "
            ...
            try:
                ...
                logger.warning(
                    "MultiCohortT1CRFlowLatentDataset: skipping cohort %s — %s",
                    name, exc,
                )
                continue
            if len(ds) == 0:
                logger.warning(
                    "MultiCohortT1CRFlowLatentDataset: cohort %s has 0 patients "
        ...
        logger.info(
            "MultiCohortT1CRFlowLatentDataset[%s/fold%d]: %d cohorts, "
