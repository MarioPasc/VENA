"""Per-cohort and multi-cohort latent-H5 datasets for the 3D-DiT competitor.

Structurally identical to ``vena.competitors.t1c_rflow.dataset`` (same data
contract: many-to-one latent → latent, conditioning latents concatenated
along the channel axis by the runner). Duplicated rather than re-exported so
the ``vena.competitors`` leaves stay independent
...
Carry-overs from the pGAN / T1C-RFlow integrations:
...
class DiT3DLatentDataset(Dataset[dict[str, torch.Tensor | str]]):
    """Per-cohort latent-H5 reader for 3D-DiT training.
    ...
    input_latents:
        ...
        — paper-faithful for the latent-diffusion baseline (T1pre + FLAIR
        surrogate for BraTS T2-FLAIR; cf. ``UPSTREAM.md`` "Scope of use").
    target_latent:
    ...
    def __init__(
        ...
        if phase not in {"train", "val", "test"}:
            raise DatasetError(
                f"phase must be one of train/val/test, got {phase!r}"
            )
        ...
        with h5py.File(self.latent_h5, "r") as f:
            ...
            if key is None:
                raise DatasetError(
                    f"none of {candidates} present in {self.latent_h5}"
                )
        ...
        logger.info(
            "DiT3DLatentDataset[%s/fold%d]: %d patients, cond=%s → %s",
            phase, fold, len(self.patient_ids),
            self.input_latents, self.target_latent,
        )
    ...
    def __getitem__(self, i: int) -> dict[str, torch.Tensor | str]:
        ...
        tgt = np.asarray(
            f[f"latents/{self.target_latent}"][pidx], dtype=np.float32
        )
...
class MultiCohortDiT3DLatentDataset(Dataset[dict[str, torch.Tensor | str]]):
    """ConcatDataset over per-cohort ``DiT3DLatentDataset`` instances.
    ...
    pGAN-cGAN and T1C-RFlow that lets server-3 / loginexa / Picasso share one
    corpus JSON when each platform mirrors only a subset locally.

    A 3D-DiT-specific invariant: every accepted cohort must produce latents
    at the same spatial shape so the DiT's fixed-size positional embedding
    is valid. The first batch yielded by the DataLoader is the contract
    check; the runner validates it before starting training.
    """
    ...
    def __init__(
        ...
        self.path_overrides = {
            k: Path(v) for k, v in (path_overrides or {}).items()
        }

        cohorts = _load_corpus_registry(self.corpus_registry)
        datasets: list[DiT3DLatentDataset] = []
        ...
        for entry in cohorts:
            ...
            if "latent_h5" not in entry:
                logger.warning(
                    "MultiCohortDiT3DLatentDataset: skipping cohort %s — "
                    "no 'latent_h5' field in registry entry", name,
                )
                continue
            ...
            if not h5.is_file():
                logger.warning(
                    "MultiCohortDiT3DLatentDataset: skipping cohort %s — "
                    "latent H5 missing at %s", name, h5,
                )
                continue
            try:
                ...
                logger.warning(
                    "MultiCohortDiT3DLatentDataset: skipping cohort %s — %s",
                    name, exc,
                )
                continue
            if len(ds) == 0:
                logger.warning(
                    "MultiCohortDiT3DLatentDataset: cohort %s has 0 patients "
                    "for fold=%d phase=%s — skipped", name, fold, phase,
                )
                continue
            datasets.append(ds)
        ...
        logger.info(
            "MultiCohortDiT3DLatentDataset[%s/fold%d]: %d cohorts, "
            "%d total patients (per-cohort: %s)",
            phase, fold, len(datasets), len(self._concat),
            ", ".join(
                f"{n}={s}" for n, s in zip(self.cohort_names, self.cohort_sizes)
            ),
        )
