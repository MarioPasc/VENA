"""Cohort image-H5 → ResViT 2D-slice dataset (single- and multi-cohort).
...
4. Pad H/W to ``image_size`` (default 256×256, divisible by 16 as required
   by ResViT's CNN encoder downsample (×4) + ViT 16×16 patch flatten in the
   ART bottleneck). Native shapes vary across cohorts:
   UCSF-PDGM/UPENN-GBM/IvyGAP/REMBRANDT are (240, 240, 155),
   BraTS-GLI/LUMIERE are (182, 218, 182). The centred pad handles both.
5. Rescale ``[0, 1] → [-1, 1]`` to match ResViT's tanh output range.

Returned dict matches the upstream ResViT ``CreateDataset`` contract:
...
class UCSFPDGMSliceDataset(Dataset[dict[str, torch.Tensor | str]]):
    ...
    def __init__(
        ...
        if image_size % 16 != 0:
            raise DatasetError(
                f"image_size must be divisible by 16 (ResViT CNN ×4 + ViT 16×16 patch); "
                f"got {image_size}"
            )
...
class CustomDatasetDataLoader:
    """Mirror ResViT upstream's ``CreateDataLoader`` API for any code path that
    happens to consume it; VENA's runner builds a plain ``DataLoader`` directly.
    """

    name = lambda self: "VENAResViTSliceLoader"  # noqa: E731
    ...
    def load_data(self):  # noqa: D401 — upstream's exact method name.
        return self
