"""NIfTI → H5 converter for the BraTS-PED 2024 image-domain cache.

Streaming layout
----------------

1. Index source patients via :class:`BraTSPedDataset`.
...
4. Dispatch one task per patient to a joblib worker pool; each task loads the
   four modalities (already HD-BET skull-stripped) + the BraTS-2023 tumour
   seg, reorients to LPS, derives the brain mask from the union of non-zero
   voxels across the four modalities (post-strip background is exactly 0),
   and computes the brain-centred crop origin.
...
from vena.data.niigz.brats_ped import BraTSPedDataset
...
_PRODUCER = f"vena.data.h5.brats_ped.image_domain.convert:{_PRODUCER_VERSION}"
_LPS: tuple[str, str, str] = ("L", "P", "S")
_COHORT_TAG = "BraTS-PED"
...
class BraTSPedImageH5Config(BaseModel):
...
def _load_patient_payload(
    ...
    for slug, suffix in BRATS_PED_IMAGE_SEQUENCE_MAP.items():
...
class BraTSPedImageH5Converter:
    """Run one end-to-end conversion of BraTS-PED to image-domain H5."""

    def __init__(self, cfg: BraTSPedImageH5Config) -> None:
        self.cfg = cfg

    def run(self) -> Path:
        cfg = self.cfg
        dataset = BraTSPedDataset(cfg.source_root)
        ...
        n = len(patient_ids)
        logger.info("BraTS-PED conversion: n_patients=%d", n)

        manifest = build_brats_ped_image_manifest(cohort_tag=_COHORT_TAG)
        ...
        with H5Writer(
            ...
            extra_root_attrs={
                "split_role": "external",
                "longitudinal": False,
                "label_system": BRATS_PED_LABEL_SYSTEM,
                "crop_box": json.dumps(list(cfg.crop_box)),
                "orientation": "LPS",
                "subset_label": "BraTS-PED-2024-Training",
            },
            ...
            image_dsets = {
                slug: w.create_stacked(
                    ...
                    n=n,
                    spatial_shape=BRATS_PED_IMAGE_EXPECTED_SHAPE,
                )
                for slug in BRATS_PED_IMAGE_SEQUENCE_MAP
            }
            ...
            w.write_int_1d(
                ...
                description=(
                    ...
                    "(1:1 for BraTS-PED cross-sectional)."
                ),
            )
            w.write_vlen_str_1d(
                "patients/keys",
                list(patient_ids),
                description="Unique patient keys (BraTS-PED-NNNNN-NNN) in offset order.",
            )
        ...
        try:
            ...
            raise
        logger.info("Wrote BraTS-PED H5 cache: %s", cfg.output_path)
