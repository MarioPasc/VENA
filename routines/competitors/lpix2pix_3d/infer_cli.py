"""CLI entrypoint for ``vena-competitor-lpix2pix-3d-infer``.
...
The generator architecture is reconstructed from the checkpoint's
...
from vena.competitors.lpix2pix_3d.inference import run_inference


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="vena-competitor-lpix2pix-3d-infer",
        description=(
            "Run 3D-Latent-Pix2Pix (Isola et al. 2017 + Eidex et al. 2025 §4) "
            "inference on N patients of a chosen split. Single generator "
            "forward pass produces the predicted latent; the frozen "
            "MAISI-V2 VAE decodes to image space; metrics are computed under "
            "VENA's percentile-norm parity contract."
        ),
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
        required=True,
        help="trained run directory under experiments/competitors/lpix2pix_3d/",
    )
    parser.add_argument(
        "--image-h5",
        type=Path,
