"""CLI entrypoint for ``vena-competitor-pgan-infer``.
...
Usage:
    ...
    vena-competitor-pgan-infer \\
        ...
        --epoch latest --n-patients 10 --phase val
"""
...
from vena.competitors.pgan_cgan.inference import run_inference


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="vena-competitor-pgan-infer",
        description="Run pGAN inference on N patients of a chosen split.",
    )
    parser.add_argument("--run-dir", type=Path, required=True,
                        help="trained run directory under experiments/competitors/pgan_cgan/")
    parser.add_argument("--image-h5", type=Path, required=True,
                        help="UCSF-PDGM image H5 path")
