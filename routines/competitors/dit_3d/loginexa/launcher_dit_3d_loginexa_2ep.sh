#!/usr/bin/env bash
# Launch the 3D-DiT 2-epoch smoke on the Picasso loginexa V100 interactive node.
#
# Invocation: run this from the Picasso login node. It SSH-hops into loginexa,
# starts a detached tmux session, and returns immediately. 3D-DiT has no VGG
...
#
# Usage:
#   bash launcher_dit_3d_loginexa_2ep.sh             # submit
#   bash launcher_dit_3d_loginexa_2ep.sh --dry-run   # print plan, no ssh
#
...
CONFIG_PATH="${REPO_DIR_REMOTE}/routines/competitors/dit_3d/configs/smoke_loginexa_2ep.yaml"
LOG_DIR="${LOG_DIR:-/mnt/home/users/tic_163_uma/mpascual/execs/vena/logs/competitors/dit_3d}"
SESSION="${SESSION:-vena-dit-3d-loginexa-smoke}"
...
&& '${PYTHON}' -m routines.competitors.dit_3d.cli '${CONFIG_PATH}' 2>&1 | tee -a '${LOG_DIR}/${SESSION}.log'"
