#!/usr/bin/env bash
# Submit the full SynDiff flair → t1c training to Picasso A100 DGX.
#
# Usage:
#   bash launcher_syndiff_picasso_full_flair.sh             # submit
#   bash launcher_syndiff_picasso_full_flair.sh --dry-run   # print sbatch command, no submit
...
export CONFIG_PATH="${REPO_DIR}/routines/competitors/syndiff/configs/picasso_full_flair.yaml"
...
SBATCH_CMD="sbatch --parsable \
    --output=${LOGS_DIR}/syndiff_picasso_full_flair_%j.out \
    --error=${LOGS_DIR}/syndiff_picasso_full_flair_%j.err \
    ...
    ${SCRIPT_DIR}/worker_syndiff_picasso_full_flair.sh"
...
echo "Submitted job ${JOB_ID} (SynDiff flair → t1c full training)"
echo "Monitor:  squeue -j ${JOB_ID}"
echo "Logs:     ${LOGS_DIR}/syndiff_picasso_full_flair_${JOB_ID}.{out,err}"
