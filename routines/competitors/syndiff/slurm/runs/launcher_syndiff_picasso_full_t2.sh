#!/usr/bin/env bash
# Submit the full SynDiff t2 → t1c training to Picasso A100 DGX.
#
# Usage:
#   bash launcher_syndiff_picasso_full_t2.sh             # submit
#   bash launcher_syndiff_picasso_full_t2.sh --dry-run   # print sbatch command, no submit
...
export CONFIG_PATH="${REPO_DIR}/routines/competitors/syndiff/configs/picasso_full_t2.yaml"
...
SBATCH_CMD="sbatch --parsable \
    --output=${LOGS_DIR}/syndiff_picasso_full_t2_%j.out \
    --error=${LOGS_DIR}/syndiff_picasso_full_t2_%j.err \
    ...
    ${SCRIPT_DIR}/worker_syndiff_picasso_full_t2.sh"
...
echo "Submitted job ${JOB_ID} (SynDiff t2 → t1c full training)"
echo "Monitor:  squeue -j ${JOB_ID}"
echo "Logs:     ${LOGS_DIR}/syndiff_picasso_full_t2_${JOB_ID}.{out,err}"
