...
# Submitted by launcher_spatial_residual_sweep.sh with --dependency=afterok:<array_id>.
# Do not submit directly.
#
# Runs on a single node; no GPU needed.  The Holm-Bonferroni correction (family
# of 16: 2 stats × 8 competitors) and the LUMIERE scan→patient collapse (72 scans
# → 11 patients) run exactly once here — never per shard.
#
...
# A merge over a partial grid produces a complete-LOOKING artifact (full tables,
# figures, a plausible n_patients) built from a fraction of the methods, and
# publishes it as LATEST — exactly what happened on 2026-07-17 (job 1604489:
# 54 of 405 files, n_patients=393, wrong).
# --allow-partial is a deliberate human decision; set ALLOW_PARTIAL=1 in the
