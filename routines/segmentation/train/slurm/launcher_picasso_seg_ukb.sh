#!/usr/bin/env bash
# ============================================================================
# Launcher — segmentation training, UKB arm, 6-task array (folds 0-4 + all_train)
# ============================================================================
# Usage:
#   bash launcher_picasso_seg_ukb.sh             # submit
#   bash launcher_picasso_seg_ukb.sh --dry-run   # print sbatch command, no submit
#
...
ARM_CONFIG="${REPO_DIR}/routines/segmentation/train/configs/runs/picasso_seg_ukb.yaml"
...
SBATCH_CMD="sbatch --parsable \
    --array=0-5 \
    --output=${LOGS_DIR}/seg_ukb_%A_%a.out \
    --error=${LOGS_DIR}/seg_ukb_%A_%a.err \
    --export=ALL,\
REPO_DIR=${REPO_DIR},\
...
echo "Submitting segmentation training array (UKB arm, 6 tasks: folds 0-4 + all_train)"
...
echo "Log pattern   : ${LOGS_DIR}/seg_ukb_${JOB_ID}_<task>.out"
echo ""
echo "Individual task logs:"
for TASK in 0 1 2 3 4 5; do
    FOLD=$( [ "${TASK}" -eq 5 ] && echo "all_train" || echo "${TASK}" )
    echo "  task ${TASK} (fold ${FOLD}): ${LOGS_DIR}/seg_ukb_${JOB_ID}_${TASK}.out"
done
