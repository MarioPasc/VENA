#!/usr/bin/env bash
# Submit the VENA MAISI encoder to Picasso.
#
# Usage (login node):
#   bash routines/encode/maisi/slurm/launcher_encode_maisi.sh
#   bash routines/encode/maisi/slurm/launcher_encode_maisi.sh path/to/other_config.yaml
#   bash routines/encode/maisi/slurm/launcher_encode_maisi.sh --dry-run
#
# Default config: configs/picasso.yaml — full UCSF-PDGM cohort, full-volume
# fp32 encode, stabilization sweep + per-modality figures + NIfTI dump.
set -euo pipefail
...
CONFIG_PATH="${CONFIG_PATH:-${REPO_DIR}/routines/encode/maisi/configs/picasso.yaml}"
...
_validate_yaml_paths() {
    ...
    # Required: source H5 and autoencoder checkpoint. Output paths are
    # created on demand by the routine, so we do not pre-validate them.
    for key in source_image_h5 autoencoder_checkpoint; do
...
JOB_NAME="vena-encode-maisi-$(date -u +%Y%m%dT%H%M%SZ)"
...
SBATCH_CMD=(
    ...
    "${SCRIPT_DIR}/worker_encode_maisi.sh"
)
