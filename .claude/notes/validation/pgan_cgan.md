...
| Field | Value | Source |
|---|---|---|
| Input modalities | `{T1pre, T2, FLAIR}` (3 channels) | UCSF-PDGM image H5 |
| Target | `T1c` (1 channel) | same H5 |
| Slice axis | Axial (z) | native (240, 240, 155) |
| Min brain voxels per slice | 1000 | drops mostly-air slices |
| Pad/crop size | 256 × 256 (centred zero-pad from 240) | pGAN's stride-2 downsamples require ÷4 |
...
| Split | `splits/cv/fold_0/{train,val}` + `splits/test` | UCSF-PDGM image H5 |

**SWAN is intentionally absent** — UCSF-PDGM's image H5 does not store SWAN. The paper's
original recipe used multi-contrast inputs anyway (T1↔T2↔PD); 3 modalities → T1c is the
direct analogue for the gadolinium-synthesis task.
...
3. **Match the channel dim contract**: pGAN expects `(B, C, H, W)` tensors with `C`
...
4. **Pre-warm any pretrained-weight caches** on each platform's login node before launching
   compute jobs. Picasso A100 nodes have no internet.
5. **Per-platform env paths**:
...
6. **Picasso SLURM requires `--constraint=dgx`** for A100 access — submitting without a
   constraint defaults to `cpu` and fails with "Requested node configuration is not
   available".
7. **The Picasso loginexa node is NOT a SLURM partition** — it's an SSH-accessible
   interactive node (10.248.7.200). Use tmux-detached SSH launches, not sbatch.
8. **`screen` vs `tmux`**: server-3 has only `screen`, loginexa has both. Sniff first or
   pick `screen` since it's more portable.
