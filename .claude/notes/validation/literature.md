...
| **3D-DiT** (added 2026-06-08) | Latent diffusion (transformer) | T1, T2, FLAIR | Adapted from Peebles & Xie 2023 / Mo et al. 2023 | No | No | Yes | **Yes** (multiple ports) | **Yes (transformer-DM tier)** |
| **T1C-RFlow 2025** | Latent RF (U-Net) | T1, T2, FLAIR | None | No | No | Yes (custom VAE) | TBD | **Yes (current SOTA)** |
...
The differentiator is now **MAISI-pretrained latent FM + $L^p$-aware contrastive on $\{T_1, T_2, \text{FLAIR}\}$** against the strongest entries of each prior era. The vessel-fidelity claim is purely loss-side (the $p_b = 3$ background term upweights non-tumour errors); the validation protocol therefore tests it through a *multi-operator vessel-extraction ensemble* applied identically to real and synthetic T1c (Frangi + Jerman + OOF + VesselFM; see `validation_proposal.md` §4.5). SWAN is **not** an input in this formulation of the problem — the previous SWAN-conditioned variant is parked as a follow-up paper once HRUM Tier 3 lands.
...
**Mandatory tier (five competitors, each re-trained on our cohort split for a controlled head-to-head):**
...
| 4 | **3D-DiT** (Peebles & Xie 2023 backbone; 3D extension per Mo et al. 2023 / Eidex et al. 2025 baseline) | Latent diffusion, transformer backbone | Tests the *transformer-backbone diffusion* axis that T1C-RFlow's U-Net does not; T1C-RFlow themselves use DiT-3D as their published baseline so reviewers will expect it. Replaces CFM (no public code) in the FM/diffusion tier | Multiple public ports (`facebookresearch/DiT`, 3D adaptations in the medical-diffusion literature; Eidex's repo if available) | Trained over MAISI latents to control for the latent-space confound vs VENA (same VAE, same latent grid, same conditioning interface) |
| 5 | **T1C-RFlow** (Eidex et al. 2025) | RF (current SOTA on BraTS) | Numerical SOTA on the same task; same <10 s/volume budget; head-to-head defines the SOTA claim | arXiv:2509.24194 (check repo) | 3D latent RF with custom VAE; expensive but mandatory |
...
- **Pix2Pix-DDPM / Palette-Med** — pure conditional DDPM. *Considered and excluded* from the mandatory tier: SynDiff (C3) already covers paired conditional diffusion (it is a strict superset: adversarial-bridge diffusion = conditional DDPM + adversarial loss). Adding a pure-DDPM baseline inflates compute by ~4 d A100 for marginal architectural-diversity gain; the discussion will state the omission with this justification.
...
**Why this set is sufficient for MedIA / MICCAI:** (i) one representative per era; (ii) two icon-lab references (pGAN, ResViT, SynDiff are the *standard* cross-contrast MR benchmark suite — reviewers expect at least one of the three); (iii) the FM/diffusion tier brackets the architectural backbones (3D-DiT = transformer, T1C-RFlow = U-Net) — these were the two backbones T1C-RFlow itself benchmarked against; (iv) VENA-S1 isolates the $L^p$-contrastive contribution under a no-architectural-delta ablation; (v) all five external competitors fit a 2-week Picasso budget at 1,224 patients, 1–4 d A100 each.
