...
- **🚀 5-JOB ORACLE MATRIX SUBMITTED (2026-07-24), repo HEAD `c2b3ab0`:**
  ...
  **⚠ They are PENDING, not RUNNING** — exit criterion 4 says RUNNING and it is NOT yet met. **UPDATE (2026-07-24, same day):** J0/J1/J2 reached RUNNING on exa03/02/04 (2× A100-40GB each, constraint held live); J3/J4 still PENDING on queue. **All three verified building correctly in production:** `conditioning_total_channels=2`, WARM_START, `init_from_trunk copied=179`, 0 errors; the JOINT arms (J1/J2) additionally loaded `trunk_ema_snapshot.pt` and show optimiser `ControlNet=223 + trunk=435` + a 361 M `trunk_ema` — the single-shot warm-start path worked first try. **`offline augmentation ENABLED` with NO `MissingSoftMaskGroupError`** — the aug soft-mask cache is consumed correctly (the smoke could not test this, aug was off). **Exit criterion 5 MET on the real run:** J0 `exhaustive_val/epoch_000/metrics.csv` = **580 rows = 116 patients × 5 NFE across all 9 cohorts**; 580/580 finite `psnr_db`, 550/580 finite `psnr_db_et` (the 30 NaN are BraTS-PED, whose sub-latent TC cores have no ET — correctly NaN, not 0). J2 (tc=5) confirms the region-weighted `reduction: none` loss runs in production. **S2 row stays `◐` only because J3/J4 have not started** — every other criterion holds. SLURM's start estimate
