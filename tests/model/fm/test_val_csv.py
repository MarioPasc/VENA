...
@pytest.mark.unit
def test_val_csv_flushes_accumulator(tmp_path: Path) -> None:
    p = tmp_path / "val.csv"
    cb = ValMetricsCSV(p)
    module = MagicMock()
    module._val_accumulator = {
        (5, "full"): {
            "mse_latent_mean": 0.5, "mse_latent_std": 0.1,
            "l1_latent_mean": 0.3, "l1_latent_std": 0.05,
            "cosine_latent_mean": 0.97,
            "psnr_image_mean": 28.0, "psnr_image_std": 1.0,
            "ssim_image_mean": 0.88, "ssim_image_std": 0.02,
            "n_patients": 4,
        }
    }
    trainer = MagicMock()
