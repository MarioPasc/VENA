"""Tests for the 3D-Latent-Pix2Pix VENA dataset wrapper.
...
Citation: arXiv:1611.07004 (Pix2Pix); arXiv:2509.24194 (3D-latent baseline).
"""
...
# VENA's actual latent grid (4×48×56×48 on the multi-cohort corpus).
_N = 4
_C = 4
_LH, _LW, _LD = 8, 8, 6
    ...
    *,
    ...
    with h5py.File(out, "w") as f:
        ...
        for mod in modalities:
            arr = rng.standard_normal(size=(n, _C, _LH, _LW, _LD)).astype(np.float32)
            for i in range(n):
                arr[i] *= (i + 1) * 0.1
        ...
        if flat_splits:
            ...
            if longitudinal:
                pats = sorted({s.decode().rsplit("-", 1)[0] for s in ids})
                train = [pats[0].encode()]
            ...
            f.create_dataset("splits/cv/fold_0/train", data=np.asarray(train))
            if val:
                f.create_dataset("splits/cv/fold_0/val", data=np.asarray(val))
            if test:
...
def test_dataset_returns_expected_keys_and_shapes(synth_h5: Path) -> None:
    ds = Pix2PixLatentDataset(latent_h5=synth_h5, fold=0, phase="train")
    sample = ds[0]
...
def test_dataset_rejects_invalid_phase(synth_h5: Path) -> None:
    with pytest.raises(DatasetError):
        Pix2PixLatentDataset(latent_h5=synth_h5, fold=0, phase="trainval")


def test_dataset_rejects_missing_latent_dataset(tmp_path: Path) -> None:
    out = _make_synth_latent_h5(tmp_path / "no_flair.h5", modalities=("t1pre", "t1c"))
    with pytest.raises(DatasetError, match="flair"):
        Pix2PixLatentDataset(latent_h5=out, fold=0, phase="train")
...
def test_dataset_falls_back_to_flat_splits_schema(tmp_path: Path) -> None:
    ...
    ds = Pix2PixLatentDataset(latent_h5=out, fold=0, phase="train")
    assert len(ds) == 2
    ds_val = Pix2PixLatentDataset(latent_h5=out, fold=0, phase="val")
    assert len(ds_val) == 1
...
def test_dataset_resolves_longitudinal_patient_ids(tmp_path: Path) -> None:
    ...
    ds = Pix2PixLatentDataset(latent_h5=out, fold=0, phase="train")
    assert len(ds) == 2  # one split id (patient) expands to 2 scans


def test_dataset_max_patients_caps_split(synth_h5: Path) -> None:
    ds = Pix2PixLatentDataset(latent_h5=synth_h5, fold=0, phase="train", max_patients=1)
    assert len(ds) == 1
