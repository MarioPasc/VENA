"""Tests for the 3D-DiT VENA dataset wrapper.
...
Citation: arXiv:2212.09748 (DiT); arXiv:2509.24194 (3D-DiT baseline recipe).
"""
...
from vena.competitors.dit_3d.dataset import (
    DatasetError,
    DiT3DLatentDataset,
)
...
def test_dataset_returns_expected_keys_and_shapes(synth_h5: Path) -> None:
    ds = DiT3DLatentDataset(latent_h5=synth_h5, fold=0, phase="train")
    sample = ds[0]
...
def test_dataset_rejects_invalid_phase(synth_h5: Path) -> None:
    with pytest.raises(DatasetError):
        DiT3DLatentDataset(latent_h5=synth_h5, fold=0, phase="trainval")


def test_dataset_rejects_missing_latent_dataset(tmp_path: Path) -> None:
    out = _make_synth_latent_h5(
        tmp_path / "no_flair.h5", modalities=("t1pre", "t1c")
    )
    with pytest.raises(DatasetError, match="flair"):
        DiT3DLatentDataset(latent_h5=out, fold=0, phase="train")
...
def test_dataset_falls_back_to_flat_splits_schema(tmp_path: Path) -> None:
    ...
    ds = DiT3DLatentDataset(latent_h5=out, fold=0, phase="train")
    assert len(ds) == 2
    ds_val = DiT3DLatentDataset(latent_h5=out, fold=0, phase="val")
    assert len(ds_val) == 1
