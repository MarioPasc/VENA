"""Tests for the multi-cohort 3D-DiT dataset (corpus_registry path).
...
def test_multicohort_concats_two_cohorts(tmp_path: Path) -> None:
    ...
    ds = MultiCohortDiT3DLatentDataset(corpus_registry=reg, fold=0, phase="train")
...
def test_multicohort_raises_when_all_cohorts_dropped(tmp_path: Path) -> None:
    ...
    with pytest.raises(DatasetError, match="no usable cohorts"):
        MultiCohortDiT3DLatentDataset(corpus_registry=reg, fold=0, phase="train")
...
def test_multicohort_path_overrides_redirect_per_cohort(tmp_path: Path) -> None:
    ...
    ds = MultiCohortDiT3DLatentDataset(
        corpus_registry=reg,
        fold=0,
        phase="train",
