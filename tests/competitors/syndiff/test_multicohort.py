"""Tests for ``MultiCohortSynDiffSliceDataset``.

Mirrors pgan_cgan/test_multicohort but adapted to SynDiff's 2-tuple return
contract and single-source dataset signature.
"""
...
def test_multicohort_concats_two_cohorts(two_cohort_corpus: Path) -> None:
    ds = MultiCohortSynDiffSliceDataset(
...
def test_multicohort_sample_shape_consistent(two_cohort_corpus: Path) -> None:
    ...
    for i in range(min(20, len(ds))):
        target, source = ds[i]
        target_shapes.add(tuple(target.shape))
        source_shapes.add(tuple(source.shape))
    # Both cohorts have different native (H, W) but must produce (1, 256, 256).
    assert target_shapes == {(1, 256, 256)}
    assert source_shapes == {(1, 256, 256)}
