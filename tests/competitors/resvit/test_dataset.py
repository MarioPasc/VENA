"""Tests for the ResViT VENA dataset wrapper.
...
def synth_h5(tmp_path: Path) -> Path:
    ...
    with h5py.File(out, "w") as f:
        ...
        f.create_dataset("splits/test",
                         data=np.array([b"UCSF-PDGM-0003"]))
...
def test_dataset_uses_per_patient_thresholds(synth_h5: Path) -> None:
    """Slices from different patients land in [-1, 1] after per-patient norm,
    independent of the raw intensity scale."""
    ds = UCSFPDGMSliceDataset(
        image_h5=synth_h5, fold=0, phase="train", image_size=256,
    )
...
def test_dataset_rejects_non_div16_image_size(synth_h5: Path) -> None:
    """ResViT needs image_size % 16 == 0 (CNN ×4 + ViT 16×16 patch)."""
    with pytest.raises(DatasetError):
        # 244 is divisible by 4 but not by 16 — should fail.
        UCSFPDGMSliceDataset(image_h5=synth_h5, fold=0, phase="train", image_size=244)
    with pytest.raises(DatasetError):
...
def test_dataset_resolves_longitudinal_patient_ids(tmp_path) -> None:
    ...
    with h5py.File(out, "w") as f:
        ...
        f.create_dataset("splits/cv/fold_0/train",
                         data=np.array([b"PT-0000", b"PT-0001"]))
